...
def ppo(
    ...
    train_sampler = CartesianSeqSampler(
        num_trjs, trj_len, seq_len + 1, batch_size, device=device
    )
    test_sampler = CartesianSeqSampler(
        num_trjs, trj_len, seq_len + 1, test_batch_size, device=device, train=False
    )
    ...
    def validate_pi(obs, act, adv, logp_old):
        ...
        with torch.no_grad():
            for batch in test_sampler:
                prev_batch = (batch[0], batch[1][:-1, :])
                last_batch = (batch[0], batch[1][-1, :])
                s = (obs[last_batch], act[prev_batch].long())
                pi, logp = ac.pi(s, act[last_batch])
                ratio = torch.exp(logp - logp_old[last_batch])
                clip_adv = (
                    torch.clamp(ratio, 1 - clip_ratio, 1 + clip_ratio) * adv[last_batch]
                )
                loss_pi += -(torch.min(ratio * adv[last_batch], clip_adv)).sum()

                # Useful extra info
                approx_kl += (logp_old[last_batch] - logp).sum().item()
    ...
    def validate_v(obs, act, ret):
        ...
        with torch.no_grad():
            for batch in test_sampler:
                ...
                last_batch = (batch[0], batch[1][-1, :])
                s = (obs[last_batch], act[prev_batch].long())
                loss_v += ((ac.v(s) - ret[last_batch]) ** 2).sum()
    ...
    def update():
        ...
        v_l_old = validate_v(obs, act, ret).item()
        ...
        for i in range(train_pi_iters):
            ...
            loss_pi, pi_info = compute_loss_pi(
                s, act[last_batch], adv[last_batch], logp_old[last_batch]
            )
            kl = pi_info["kl"]
            if kl > 1.5 * target_kl:
        ...
        for i in range(train_v_iters):
            ...
            last_batch = (batch[0], batch[1][-1, :])
            s = (obs[last_batch], act[prev_batch].long())
            loss_v = compute_loss_v(s, ret[last_batch])
            loss_v.backward()
            vf_optimizer.step()

        loss_pi, pi_info = validate_pi(obs, act, adv, logp_old)
        loss_pi = loss_pi.item()
        loss_v = validate_v(obs, act, ret).item()
    ...
    for epoch in range(epochs):
        for t in range(trj_len):
            ...
            hist_act = torch.as_tensor(
                env.get_act_buffer(), dtype=torch.long, device=device
            )
            s = (o_t, hist_act)
            a, v, logp = ac.step(s)
