...
class BatchSampler:
    """Random subset sampling from {0, 1, ..., M-1} X {0, 1, ..., L-1} where X is Cartesian product.
    Attributes:
    ...
    def __next__(self):
        if self.training:
            ...
            traj_idx = torch.randint(0, self.L, (self.batch_size,), device=self.device)
            ...
            if prev_idx >= self.size:
                ...
                next_idx = self.size
            ens_idx = torch.arange(prev_idx, next_idx, device=self.device) // self.L
            traj_idx = torch.arange(prev_idx, next_idx, device=self.device) % self.L
            self.index += 1
            return (ens_idx, traj_idx)


class CartesianSampler(BatchSampler):
    ...
    def __init__(self, M, L, batch_size, device="cpu", train=True):
        super(CartesianSampler, self).__init__(
